...
export default function BlogPage() {
  ...
  const allBlogs = useMemo(
    () => ((blogsData?.data ?? []) as Blog[]).filter((b) => b.isPublished !== false),
    [blogsData],
  );
  ...
  const featuredPost = page === 1 && activeCategory === "All" && filtered.length > 0 ? filtered[0] : null;
  const remainingPosts = featuredPost ? filtered.slice(1) : filtered;

  return (
    <DefaultPage>
      {/* Hero */}
      <div className="relative overflow-hidden bg-gradient-to-br from-[var(--bg-secondary)] to-[var(--bg-primary)] py-14 sm:py-20 border-b border-[var(--border-primary)]">
          ...
          <div className="mx-auto mt-6 max-w-md">
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-full border border-[var(--border-primary)] bg-[var(--bg-card)] py-2.5 pl-11 pr-4 text-sm text-[var(--text-primary)] placeholder:text-[var(--text-muted)] outline-none focus:border-[var(--accent-primary)]"
              />
            </div>
          </div>
        </div>
