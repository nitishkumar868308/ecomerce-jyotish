...
import { useSetAstrologerCommission } from "@/services/admin/jyotish";
...
export function SetCommissionModal({ astrologer, isOpen, onClose }: Props) {
  const [commission, setCommission] = useState<string>("20");
  const { mutate, isPending } = useSetAstrologerCommission();

  useEffect(() => {
    if (astrologer?.commissionPercent != null) {
      setCommission(String(astrologer.commissionPercent));
    }
  }, [astrologer?.id, astrologer?.commissionPercent]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-md rounded-2xl bg-[var(--bg-primary)] p-6 shadow-xl">
        <h3 className="text-lg font-semibold text-[var(--text-primary)]">
          Commission for {astrologer.name}
        </h3>
